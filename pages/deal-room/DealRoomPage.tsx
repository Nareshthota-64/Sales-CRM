...
// FIX: Changed the type of the `icon` prop from `React.ReactElement` to `React.ReactNode` and added `React.isValidElement` as a type guard to safely clone the icon and add a className. This resolves the TypeScript error.
const Widget: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode; delay: number }> = ({ title, icon, children, delay }) => (
    <div className="bg-white p-6 rounded-2xl shadow-sm animate-fade-in" style={{ animationDelay: `${delay}ms`}}>
        <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
            {React.isValidElement(icon) ? React.cloneElement(icon, { className: 'w-5 h-5 text-slate-500' }) : icon}
            {title}
        </h2>
        {children}
    </div>
